...
async function loadPerfumes() {
  ...
  try {
    ...
    if (perfumes.length === 0) {
      adminProductsContainer.innerHTML = `<p class="text-stone-400 text-sm text-center col-span-full">No perfumes found in catalog.</p>`;
      return;
    }
    ...
    adminProductsContainer.innerHTML = perfumes.map(perfume => `
      <div class="relative bg-stone-900 border border-stone-800 rounded-2xl p-4 flex flex-col justify-between overflow-hidden shadow-xl">
        <button onclick="deleteProduct('${perfume.id}')" 
            ...
            class="absolute top-3 right-3 p-2 bg-white/90 hover:bg-red-500 rounded-full transition-colors z-10 group">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-gray-700 group-hover:text-white">
